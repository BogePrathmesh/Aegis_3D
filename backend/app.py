...
# New feature scripts
from scripts.stitcher import stitch_images
...
def upload_image():
    ...
    for file in files:
        ...
        filename = f"{session_id}_{len(uploaded_paths)}_{uuid.uuid4().hex[:4]}.{ext}"
    ...
    # Image Stitching - Only if multiple images
    master_image_path = uploaded_paths[0]
    if len(uploaded_paths) > 1:
        stitched_path = stitch_images(uploaded_paths, UPLOAD_FOLDER)
        if stitched_path:
            master_image_path = stitched_path
        else:
            # Fallback to the first image if stitching fails
            print("Stitching failed, falling back to first image.")
            master_image_path = uploaded_paths[0]

    # Standardize original name for simulation engine (mandatory _original suffix)
    master_ext = master_image_path.rsplit(".", 1)[1].lower()
    final_original_name = f"{session_id}_original.{master_ext}"
    final_original_path = os.path.join(UPLOAD_FOLDER, final_original_name)
    
    # If it's not already named correctly, copy it to the _original path
    if os.path.abspath(master_image_path) != os.path.abspath(final_original_path):
        import shutil
        shutil.copy(master_image_path, final_original_path)

    # Run Inference with GPS Extraction
    inference_results = run_inference(final_original_path)
    
    # Save the mask generated by inference
    mask_filename = f"{session_id}_mask.png"
    cv2.imwrite(os.path.join(MASK_FOLDER, mask_filename), inference_results['mask'])

    # Generate depth map
    depth = generate_depth_map(final_original_path, inference_results['mask'])
    depth_filename = None
    ...
    img = cv2.imread(final_original_path)
    h, w = img.shape[:2]

    return jsonify(
        {
            "session_id": session_id,
            "filename": final_original_name,
            "mask_filename": mask_filename,
            "depth_filename": depth_filename,
            "width": w,
            "height": h,
            "latitude": inference_results['latitude'],
            "longitude": inference_results['longitude'],
            "gps_status": inference_results['gps_status'],
            "crack_properties": inference_results['crack_properties'],
        }
    )
...
def run_upload_simulation(session_id):
    ...
    if not (orig_path and os.path.exists(mask_path)):
        return jsonify({"error": f"Assets not found for session {session_id}"}), 404
        
    # Re-extract GPS and properties from original assets for simulation
    from scripts.inference import extract_gps_data, analyze_crack_properties
    lat, lng, status = extract_gps_data(orig_path)
